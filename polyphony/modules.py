...
from polyphony.timing import timed, clkfence, clksleep, wait_value, wait_until
...
class Handshake:
    ...
    def rd(self):
        ...
        self.ready.wr(True)
        clkfence()
        wait_until(lambda : self.valid.rd() == True)
        self.ready.wr(False)
        return self.data.rd()
    ...
    def wr(self, v):
        ...
        self.valid.wr(True)
        clkfence()
        wait_until(lambda : self.ready.rd() == True)
        self.valid.wr(False)
...
class RAMModule:
    def __init__(self, dtype, capacity, delay):
        ...
        self.port.q.assign(lambda:self.mem[self.port.addr.rd()])
