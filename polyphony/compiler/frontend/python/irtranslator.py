...
from ...ir.irhelper import eval_unop, eval_binop, eval_relop
...
class ScopeVisitor(ast.NodeVisitor):
    ...
    def visit_Name(self, node):
        ...
        if ancestor and ancestor.scope.name == "polyphony" and ancestor.name == "__python__":
            return False
        return None
    ...
    def visit_ClassDef(self, node):
        ...
        if outer_scope.is_lib() and "inlinelib" not in tags:
            tags.add("lib")
        self.current_scope = Scope.create(outer_scope, node.name, tags | {"class"}, node.lineno)
        self.current_scope.synth_params.update(synth_params)
        if self.current_scope.parent.synth_params["scheduling"] == "timed":
            self.current_scope.synth_params.update({"scheduling": "timed"})
...
class CodeVisitor(ast.NodeVisitor):
    ...
    def _needJUMP(self, block):
        ...
        if not block.stms:
            return True
        last = block.stms[-1]
        return not isinstance(last, Jump) and not (
            isinstance(last, Move)
            and isinstance(last.dst, Temp)
            and self.current_scope.find_sym(last.dst.name).is_return()
        )
    ...
    def _visit_lazy_FunctionDef(self, node):
        ...
        if self.current_scope.is_method():
            if not params or not params[0][0].is_param():
        ...
        if self.current_scope.is_lib() and not self.current_scope.is_inlinelib():
            ...
            if scope_sym:
                scope_sym.typ = Type.function(
                    self.current_scope, self.current_scope.return_type, self.current_scope.param_types()
                )
                ...
                if isinstance(right, Array):
                    ...
                    for item in right.items:
                        if not (isinstance(item, Const) and isinstance(item.value, str)):
                            fail((env.current_filename, node.lineno), Errors.MUST_BE_X, ["string literal"])
                        imp_name = item.value
                        self.current_scope.all_imports.append(imp_name)
                else:
                    fail((env.current_filename, node.lineno), Errors.MUST_BE_X, ["A sequence of string literal"])
    ...
    def visit_While(self, node):
        ...
        if not self.current_scope.is_inlinelib() and self.current_scope.synth_params["scheduling"] == "timed":
            fail((env.current_filename, node.lineno), Errors.RULE_TIMED_WHILE_LOOP_IS_NOT_ALLOWED)
    ...
    def visit_For(self, node):
        ...
        while is_iterator_func(it):
            ...
            if it.name == "polyphony.unroll":
                if is_iterator_func(seq) and seq.name in ("polyphony.unroll", "polyphony.pipelined"):
                    fail((env.current_filename, node.lineno), Errors.INCOMPATIBLE_PARAMETER_TYPE, [seq.name, it.name])
                if len(it.args) == 1:
                    kw = dict(it.kwargs)
                    if len(kw) == 0:
                        ...
                        kwarg = next(iter(kw))
                        fail((env.current_filename, node.lineno), Errors.GOT_UNEXPECTED_KWARGS, [it.name, kwarg])
                elif len(it.args) == 2:
                    _, factor = it.args[1]
                else:
                    fail((env.current_filename, node.lineno), Errors.TAKES_TOOMANY_ARGS, [it.name, "2", len(it.args)])
                ...
                if is_iterator_func(seq):
                    if seq.name == "polyphony.pipelined":
                        fail(
                            (env.current_filename, node.lineno), Errors.INCOMPATIBLE_PARAMETER_TYPE, [seq.name, it.name]
                        )
                if len(it.args) == 1:
                    kw = dict(it.kwargs)
                    if len(kw) == 0:
        ...
        if self.current_scope.synth_params["scheduling"] == "timed" and not (
            isinstance(it, SysCall) and it.name == "polyphony.timing.clkrange"
        ):
            fail((env.current_filename, node.lineno), Errors.RULE_TIMED_FOR_LOOP_IS_NOT_ALLOWED)
        ...
        if isinstance(it, SysCall) and it.name == "range":
            ...
            init_parts += [Move(dst=Temp(name=var.name, ctx=Ctx.STORE), src=start)]
            condition = RelOp(op="Lt", left=Temp(name=var.name), right=end)
            continue_parts = [
                Move(dst=Temp(name=var.name, ctx=Ctx.STORE), src=BinOp(op="Add", left=Temp(name=var.name), right=step))
            ]
            ...
            start = Const(value=0)
                ...
                name="", func=Temp(name="len", ctx=Ctx.CALL), args=[("seq", it.model_copy(deep=True))], kwargs={}
            )
            counter_name = Symbol.unique_name("@counter")
            counter = self.current_scope.add_sym(counter_name, tags=set(), typ=Type.undef())
            init_parts = [Move(dst=Temp(name=counter_name, ctx=Ctx.STORE), src=start)]
            condition = RelOp(op="Lt", left=Temp(name=counter_name), right=end)
            body_parts = [
                Move(
                    dst=Temp(name=var.name, ctx=Ctx.STORE),
                    src=MRef(mem=it.model_copy(deep=True), offset=Temp(name=counter_name), ctx=Ctx.LOAD),
                )
            ]
            continue_parts = [
                Move(
                    dst=Temp(name=counter_name, ctx=Ctx.STORE),
                    src=BinOp(op="Add", left=Temp(name=counter_name), right=Const(value=1)),
                )
            ]
            ...
            start = Const(value=0)
                ...
                name="", func=Temp(name="len", ctx=Ctx.CALL), args=[("seq", Temp(name=unnamed_array.name))], kwargs={}
            )
            counter_name = Symbol.unique_name("@counter")
            counter = self.current_scope.add_sym(counter_name, tags=set(), typ=Type.undef())
            init_parts = [
            ...
            body_parts = [
                Move(
                    dst=Temp(name=var.name, ctx=Ctx.STORE),
                    src=MRef(mem=Temp(name=unnamed_array.name), offset=Temp(name=counter_name), ctx=Ctx.LOAD),
                )
            ]
            continue_parts = [
                Move(
            ...
            fail((env.current_filename, node.lineno), Errors.UNSUPPORTED_SYNTAX, ["This type of for statement"])
        if counter:
    ...
    def visit_Assert(self, node):
        ...
        self.emit(
            Expr(exp=SysCall(name="", func=Temp(name="assert", ctx=Ctx.CALL), args=[("exp", testexp)], kwargs={})), node
        )
        ...
        if isinstance(func, Temp):
            ...
            if not func_scope:
                if func.name in dir(python_builtins):
                    raise NameError(
                        "The name '{}' is reserved as the name of the built-in function.".format(node.func.id)
                    )
            else:
    ...
    def visit_Attribute(self, node):
        ...
        if isinstance(value_sym, Symbol) and value_sym.typ.has_scope():
